...
export const STATUS: Record<string, StatusStyle> = {
  ...
  skipped: { node: 'n-skip', icon: '⊘', color: '#e8b34a', badge: ['SKIPPED', 'b-skip'] },
  suspect: { node: 'n-cas', icon: '?', color: '#f0a24e', badge: ['SUSPECT', 'b-sus'] },
...
export function nodeState(n: GraphNode, isTestRun: boolean): string {
...
export function badgeState(n: GraphNode): string {
  if (n.resource_type === 'source')
    return n.freshness_status === 'warn' || n.freshness_status === 'error' ? 'stale' : 'source';
  if (n.status === 'error') return 'failed';
  if (n.failure_class === 'suspect') return 'suspect';
